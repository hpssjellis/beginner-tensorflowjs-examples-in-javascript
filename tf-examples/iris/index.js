...
//import * as data from './data';   // did not use the name space data.
...
async function predictOnManualInput(model) {
  ...
  tf.tidy(() => {
   ...
   // const winner = data.IRIS_CLASSES[predictOut.argMax(-1).dataSync()[0]];   // did not use data. namespace
...
async function iris() {
  ...
  const [xTrain, yTrain, xTest, yTest] = getIrisData(0.15);   // did not use data. namespace
