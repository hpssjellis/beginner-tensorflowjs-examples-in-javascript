...
app.get('/', (req, res) => {
    myHostname = req.headers.host; // This gets the current request's hostname
   // res.send(myMake())
   res.send(myMake(myHostname));
});
...
// minor errors here that is doesn't show the URL but in gitpod you can find it
const myPort = process.env.PORT || 3000; // Use 3000 as a fallback port if process.env.PORT is not set
